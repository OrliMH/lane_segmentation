...
image_dirs = ['disk2/lane_segment/data/Road%02d/ColorImage_road%02d' % (i, i) for i in range(2, 5)] #+ \
            #  ['../dataset/round2/train/ColorImage_road%02d' % i for i in range(2, 5)]
label_dirs = ['disk2/lane_segment/data/Gray_Label/Label_road%02d' % i for i in range(2, 5)] #+ \
            #  ['../dataset/round2/train/Gray_Label/Label_road%02d' % i for i in range(2, 5)]
image_label_dirs = list(zip(image_dirs, label_dirs))
out_dir = 'disk2/lane_segment/AdamW_cosine_annealing_lane_segmentation/data_list'
